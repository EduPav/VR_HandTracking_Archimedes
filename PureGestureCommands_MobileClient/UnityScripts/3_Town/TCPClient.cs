...
public class TCPClient : MonoBehaviour
{
    ...
    // private string host="15.229.78.100";
    // private string host="192.168.56.1";
    // private string host="192.168.1.38";
    private string host=InputController.ipServer?.Replace("\u200B", "");   //Here we access the first scene class attribute
    ...
    private void Update()
    {
        if (webcamTexture.isPlaying && webcamTexture.didUpdateThisFrame)
        {
            ...
            //Send data length
            byte[] dataLength = BitConverter.GetBytes(encodedImage.Length);
            SendData(dataLength);
